...
function displayImages(reviews) {
  let reviewCard = "";
  reviews.forEach(review => {
    // Convierte y formatea la fecha usando moment.js
    const formattedDate = moment(review.created_at).format('DD/MM/YYYY, HH:mm:ss');

    // Construye la tarjeta con la fecha formateada
    reviewCard += `
    <div class="col">
      <div class="card overflow-auto shadow-lg text-center" style="height: 200px;">
        <div class="card-header">
          ${review.author_details.username}
          <span class="badge text-bg-info ms-2">${review.author_details.rating}</span>
        </div>
        <div class="card-body">
          <blockquote class="blockquote mb-0 p-3">
            <p class="lh-lg">${review.content}</p>
            <footer class="blockquote-footer">${review.author} <cite title="Source Title">${formattedDate}</cite></footer>
         </blockquote>
        </div>
      </div> 
    </div>
  ...
  recommendationsDiv.innerHTML = reviewCard;
}
